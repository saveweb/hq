...
Project 内可独立登记和移除的逻辑单元，分为可执行 `queue` 与只写入 S3 的 `receiver`。逻辑 shard ID 保持稳定且不能复用。

每个 shard 本质上是一个独立 queue；增减该 queue 不会改变其他 shard 的任务归属。
...
一种只接收 job batch 并写入 S3 的虚拟 shard，用于多阶段抓取流程。它不是可消费 queue，不分配 SQLite 和 owner，只支持 `enqueue`；`claim`、`complete`、`fail`、续租和状态更新请求全部返回 `unsupported_operation`。
...
- **冷数据面**：S3-compatible storage 保存尚未激活的 shard source、运行中的 checkpoint、receiver 输出，以及处理完成后的归档。
- **在线数据面**：第一版使用 SQLite 保存正在执行的 shard job。
- **loader**：激活 shard 时，从 `source_uri` 流式载入在线 backend，并持久化 `load_checkpoint`。
- **调度器**：只向 worker 分配 `active queue` shard，并遵守 Project 的 active queue 数量限制；receiver 永不进入 claim 候选。
- **receiver gateway**：无状态地接收 job batch，压缩并写入 receiver 的 S3 prefix，持久化成功后才响应。
...
Source、已发布 checkpoint、receiver 输出和 archive 都必须写入 S3。Checkpoint 上传并验证成功后立即删除本地临时副本。
...
- 普通 worker 不直接访问 R2。Receiver batch 由 HQ receiver gateway 限长、压缩并流式写入 R2，gateway 收到 R2 成功响应后才确认；
- 经过审核的 shard owner 可以直接上传 checkpoint。Tracker 决定不可复用的完整 object key，并签发只对该 key 有效、仅包含 multipart 所需操作的 R2 temporary credential；owner 不能 list、读取或删除其他对象；
- 每份 credential 的 TTL 在签发时确定，签发后不能修改。只要 owner lease 和 generation 仍有效，credential 过期后可以签发一份新的 credential，继续使用原 `upload_id`；已经成功的 part 不需要重传；
- 同一 shard 同时只允许一个 checkpoint upload，避免多个 checkpoint 乱序完成。Owner 使用标准 S3 SDK 管理 multipart；失联后留下的未完成 multipart 由 R2 lifecycle 清理；
...
S3/R2 credential 只是上传能力，不是 checkpoint 的提交凭证。即使旧 owner 在失联后完成了 R2 multipart，只有 tracker 中 owner、generation 和 lease 都仍有效时，checkpoint pointer 的 CAS 才能成功；否则对象只是等待 GC 的孤儿对象。

参考：[R2 S3 API](https://developers.cloudflare.com/r2/api/s3/api/)、[R2 presigned URL](https://developers.cloudflare.com/r2/api/s3/presigned-urls/)、[R2 temporary credential](https://developers.cloudflare.com/r2/api/s3/temporary-credentials/)、[R2 multipart upload](https://developers.cloudflare.com/r2/objects/upload-objects/)。
...
| `kind` | `text` | `queue` 或 `receiver`；创建后不可修改 |
| `owner_agent_id` | `text null` | 外键，指向 tracker 当前指派的 shard agent；receiver 为空 |
| `status` | `text` | `staged`、`loading`、`active`、`draining`、`paused`、`offline`、`recovering`、`completed`、`archived`、`load_failed`、`removed` |
...
| `sink_uri` | `text null` | receiver 输出前缀，例如 `s3://bucket/project/receiver-001/` |
| `sink_format` | `text null` | receiver 对象格式；第一版为 `jobs-jsonl-zstd-v1` |
| `source_etag` | `text null` | source 的版本或内容摘要，防止载入期间被替换 |
| `load_checkpoint` | `jsonb null` | loader 的断点位置和导入统计 |
...
| `generation` | `bigint null` | queue owner epoch；queue 必须非空，receiver 不使用 |
...
- `queue` shard 在进入 `loading` 前必须有 `source_uri`、固定的 `source_etag` 和 `source_format=jobs-jsonl-zstd-v1`；
- `active queue` 必须有当前 `owner_agent_id` 和有效 owner lease；对应 shard agent 必须有 tracker 已从公网验证的 HTTP(S) endpoint 和该 endpoint 所需的可选 TLS pin；
- `receiver` 必须有 `sink_uri` 和 `sink_format=jobs-jsonl-zstd-v1`，只使用 `active` 或 `removed` 状态；其 owner、owner lease 和 checkpoint 字段必须为空；
- `queue` 的 generation 必须非空且只能递增；receiver 的 generation 必须为空；
- receiver 不计入 `max_active_shards`，也不会被调度器选中；
...
- 对 `queue` shard，`(project_id, id, generation)` 唯一标识一次 owner 任期；
...
| `outcome_uri` | `text null` | 抓取结果或 WARC 文件位置 |
| `error` | `json null` | 结构化错误信息 |

Queue API v1 中，`complete` 只接受 `success`、`http_error`、`skipped` 三种 `outcome_kind` 并把 job 置为 `done`；HTTP 404 属于确定的 `http_error` outcome，不是 queue failure。网络超时、DNS 失败等执行错误使用 `fail(retryable=true)`，明确不可重试的执行错误使用 `fail(retryable=false)`。WARC 或页面正文不能放入 queue RPC，只保存 artifact URI；artifact 上传协议与 queue API 分离。
...
```text
enqueue(jobs)    将一个 batch 持久化到 S3，成功后返回对象 URI
```

每次调用生成一个独立、不可变的 `.jobs.jsonl.zst` 对象。Receiver 不在本地累计待上传任务，也不在 S3 上追加已有对象；因此 gateway 可以保持无状态。`claim`、`complete`、`fail`、`extend_lease`、`requeue_expired` 和 `scan` 均返回 `unsupported_operation`。
...
目标为 receiver 时，gateway 从控制面解析 `sink_uri`，限制 batch 大小，压缩后流式写入 R2。写入失败或结果不确定时返回可重试错误。
...
7. loader 按 generation 流式读取 S3 source，以稳定 job ID 幂等写入后端，并持续更新 `load_checkpoint`；owner 失联时必须等 lease 到期再换 owner；
8. loader 校验 source ETag 和导入统计，成功后把 shard 设为 `active`；失败则设为 `load_failed`，下次从 `load_checkpoint` 恢复；
...
5. 普通单阶段流程中，完成后原子写入 outcome、同 queue 派生任务和 `done` 状态；多阶段 receiver 流程按 8.5 节先写 S3，再完成当前 job；
...
```text
s3://<bucket>/checkpoints/<project_id>/<shard_id>/<generation>/<checkpoint_seq>-<checkpoint_upload_id>.sqlite.zst
```
...
5. Tracker 返回只对该 key 有效、只包含 multipart 必要操作的短期 temporary credential；owner 使用标准 S3 SDK 创建、上传并完成 multipart；
6. Credential TTL 不能修改。若上传尚未完成且 owner lease、generation、`checkpoint_upload_id` 仍匹配，tracker 可以签发一份新的同范围 credential，由 owner 继续原 `upload_id`；
7. Owner 报告完成后，HQ 通过 `HeadObject` 确认对象存在且实际大小匹配；
...
4. B 从 `checkpoint_uri` 下载对象，重新计算 SHA-256 并校验 SQLite 快照；如果校验失败或尚无运行时 checkpoint，则拒绝使用该对象并从不可变 `source_uri` 重新载入；
5. B 把快照中的 `wip` 和 `resetting` 全部恢复为 `todo`，清空 session、attempt ID、lease 和 `claim_generation`；基础设施故障不增加 job 的 `reset_count`；
...
2. A 手里的旧 temporary credential 可能尚未过期，因此它仍可能继续写 part，甚至完成这个精确 key 的 multipart；但它不能写其他 key，也不能修改 tracker pointer；
3. A 报告上传完成后，HQ 执行 `HeadObject`，最后以 PostgreSQL CAS 发布 checkpoint pointer。R2 complete 与 PostgreSQL 事务不能原子提交，因此 **CAS 才是 checkpoint 的提交点**；
...
3. receiver gateway 将 batch 压缩并写入唯一 S3 对象：
   ...
   ```text
   <sink_uri>/<random_id>.jobs.jsonl.zst
   ```
...
第一版不增加 outbox、manifest、seal、自动 compaction 或 Pipeline/Stage 数据模型。普通 worker 始终把 batch 交给 HQ receiver gateway，由 gateway 流式执行 S3 PUT；worker 不获得 S3 credential。所有已完成的父任务都对应一次明确成功的 receiver 写入。超时但后来才落入 R2 的未确认对象最多造成额外输入，不属于必须收集的已确认集合，下一阶段按稳定 job ID 去重。

### 8.6 用户登录、机器上线与撤销

#### 8.6.1 GitHub OAuth 登录 Tracker
...
#### 8.6.2 Machine Token 与 Agent 上线
...
用户或管理员可以在 tracker 后台重置 machine token。重置后，tracker 立即拒绝旧 token 发起的上线、心跳、租约续期、checkpoint credential 请求和 checkpoint pointer CAS，该用户的全部机器需要换成新 token。Worker 不再取得新的 session、assignment 或 shard access token；已经签发的 access token 最多继续有效到自身 `exp`，已有 job lease 自然到期。Shard 的现有 owner lease 不再续期，tracker 停止向新 worker 返回该 endpoint，等待 lease 到期后按 8.4 节递增 generation 并接管。重置 token 本身不能绕过 lease 直接制造两个同时有效的 owner。旧 owner 已取得的精确 key R2 credential 可能继续有效到原 TTL，但无法通过 tracker CAS 发布 checkpoint。

#### 8.6.3 Local Admin Token
...
- **载入失败**：进入 `load_failed`，修复问题后从 `load_checkpoint` 重新进入 `loading`；
...
- 100k 目标只计算 JobSpec、attempt、lease 和 outcome metadata；WARC、页面正文、receiver object 与 checkpoint 流量单独计量；
...
- Checkpoint 使用 multipart upload；审核过的 owner 只获得精确 checkpoint key、固定 TTL、仅包含 multipart 必要操作的 temporary credential，不能 list、get、delete 或写其他 key；credential 到期后只能重新签发，不能延长原 credential；
- 只有 owner lease、generation 和当前 `checkpoint_upload_id` 仍匹配时才能重新签发 credential 或发布 pointer；
...
4. artifact/WARC 的上传 API、幂等 object identity，以及失效 generation 对象的垃圾回收；
5. receiver 单次对象的目标大小、flush 策略和第二阶段 merge/split 工具；
6. 对浏览器无法访问 GitHub 授权页的用户，是增加独立的国内身份源，还是管理员一次性邀请码；第一版不代理 GitHub 登录页面、密码或 cookie；
7. 使用目标 tracker 机型、真实 JobSpec 和多台公网 shard 完成 100,000 completed jobs/s 验收时的 shard 数量、延迟与带宽门槛。
...
8. 实现 receiver gateway、S3 source 载入、checkpoint、generation recovery、归档和 GC；
9. 最后补充计划内物理迁移和管理页面诊断功能；其他 backend、computed routing 和逻辑重分片不属于第一版。
