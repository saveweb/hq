...
第一版不提供 Project JSON 配置。`max_resets` 是 shard daemon 的显式启动参数，默认
为 `3`；同时运行多少 shard 由管理员显式 attach/pause，而不是由隐藏调度器自动决定。
...
Project 可以随时 attach 新 queue。长期 Project 若只想一次运行一份预切分输入，由
操作者逐个 attach、drain、pause；第一版不自动激活下一份，也不实现 shard 删除。
...
| `owner_agent_id` | `text` | 外键，指向 tracker 当前指派或最后持有该 shard 的 agent |
| `status` | `text` | `loading`、`active`、`draining`、`paused`、`offline`、`recovering`、`load_failed`、`recovery_failed`；第一版正常管理链不自动写 `offline` |
...
| `owner_lease_expires_at` | `int64` | 当前 owner 的 tracker 租约截止时间，UNIX 秒；`paused` 时为 0 |
...
- `paused` shard 不得保留有效 owner lease，并且必须已有 published checkpoint；
- shard ID 在同一 Project 内永不复用；第一版不提供删除或复用；
...
Receiver 不参与 queue assignment，也没有 owner lease、generation、checkpoint 或本地
daemon。
...
1. 操作者创建长期 `explicit` Project；
2. 把 `jobs.txt` 切成多份，并由 source 工具生成带稳定 ID 的 `.jobs.jsonl.zst` 后上传 S3；
3. 尚未运行的文件只作为 R2 冷对象保存，不需要先在 tracker 创建 `staged` 状态；
4. 需要处理下一批任务时，管理员显式提交 source、ETag、owner 和 generation，原子创建 `loading` shard；
5. 控制面检查 Project、审核 owner、公网 endpoint、可选 TLS pin 和健康检查；
6. tracker 为 `loading` shard 签发 owner lease；
...
`explicit` Project 的第一版 shard 操作只有：

- `attach_shard`：用新的 `shard_id`、`source_uri`、`source_etag` 和 owner 创建一个 `loading` queue；不影响其他 shard；
- `attach_receiver`：登记 `sink_uri`，创建一个立即可写的 `active` receiver；不占 active queue 配额；
- `drain_shard`：以 generation CAS 把 `active` 设为 `draining`；owner 下次 heartbeat 后停止新 claim，已有 WIP、lease maintenance 和 checkpoint 继续；
- `resume_claims`：在 Project 仍为 `active` 时把 `draining` 恢复为 `active`，generation 不变；
- `pause_shard`：只有 `draining` 且已有 published checkpoint 时才能进入 `paused`；该事务把 owner lease 清零。管理员应先从 owner 本机页确认 WIP 已清空；
- `resume_shard`：递增 generation，从 checkpoint 恢复 `paused` queue，再进入 `active`；
- `recover_shard`：只有旧 owner lease 已过期或已通过 pause 清零时，才能以更高 generation 和指定 owner 进入 `recovering`。数据库 CAS 拒绝租约重叠。

第一版不提供 queue shard detach/delete、自动 completion、自动选择 replacement owner 或
自动激活下一份。处理完的 shard 可以保持 `paused`，整个项目结束后再把 Project 设为
`archived`。这避免为 tombstone、归档和后台调度维护另一套状态机。

Receiver 没有需要 drain 的本地 queue。停止所有生产者后即可 detach；detach 只移除逻辑入口，不自动删除已经写入 `sink_uri` 的对象。

Attach、drain、pause 和 recover 都只作用于一个显式 shard，不触发其他 shard 的重分片、复制或路由更新。
...
- paused shard 至少保留 1 份可恢复 checkpoint；第一版不自动删除 checkpoint。
...
1. tracker 的管理页显示 owner 心跳、endpoint 和 lease；第一版由管理员选择 replacement owner，不运行自动故障调度器；
2. 管理员必须等待 A 的 owner lease 到期。计划内交接则先执行 `active → draining → paused`，由 pause 事务把租约清零；
3. 管理员以 generation CAS 指派 B 并进入 `recovering`。PostgreSQL 同时检查 `generation > g`、已有 published checkpoint 和旧 lease 已失效，否则拒绝；
...
OAuth 只完成 authentication，不能自行增加 `roles`。新用户是否自动得到 `worker` role 可以作为部署策略；`shard_owner` 和 `admin` 必须由现有管理员授予。Web 上的 role、用户状态和 Project/shard/Receiver 管理操作写入 audit log。
...
```text
attach → loading → active ↔ draining → paused
           │                         │
           └→ load_failed            └→ recovering → active
                                             └→ recovery_failed
```

- **登记并载入**：管理员显式指定 source 和 owner，直接创建 `loading`，分配 SQLite 文件并流式载入；成功后进入 `active`；
- **载入失败**：进入 `load_failed`，修复问题后从头幂等载入；不维护额外的 source 断点状态；
- **执行**：调度器只从 `active` shard claim 任务；
- **暂停**：进入 `draining` 后停止新 claim；管理员确认 WIP 清零且已有 checkpoint 后显式进入 `paused`，事务清零 owner lease；
...
- **故障**：管理员等待旧 lease 到期后，以更高 generation 指派新 owner 进入 `recovering`；checkpoint 恢复完成后回到 `active`；
- **完成**：第一版没有自动 `completed` 或 shard 删除状态；处理完成后按同一 drain/pause 流程保留 checkpoint，Project 最后可设为 `archived`。
...
- Project 可以动态 attach/drain/pause shard，S3 承担冷数据容量；管理员显式控制同时在线的 shard 数量；
...
- loading、active、draining、paused、recovering、load_failed、recovery_failed shard 数量；
...
- tracker web session 使用安全 cookie 和 CSRF 防护；role/用户状态以及 Project、shard、Receiver 的管理变更必须记录操作者、时间、目标和原因；
...
- 查看 agent 类型、名称、在线状态、endpoint health 和最近心跳；
- 重置 machine token，使自己的全部机器需要更新配置。

第一版 contributor portal 不修改单机名称/状态，也不展示历史请求或 session；这些信息
先由 daemon 日志和本机管理页承担。
...
- 查看 shard agent 的 endpoint health，并在选择 owner 时使用已审核 agent；
- 创建/更新 Project，attach source shard、从 checkpoint recover，以及执行
  `active ↔ draining → paused`；
- 查看 shard 的 owner、generation、lease、source、checkpoint、载入/恢复错误和 Job Receiver；
- 查看最近 100 条管理 audit log。

页面不能绕过状态机直接编辑 PostgreSQL row。所有变更在 PostgreSQL transaction 内
重新校验 active admin、状态与 generation，并在同一 transaction 写 audit reason。
第一版不提供强制跳过 owner lease、agent 撤销、checkpoint GC 或系统告警页面。

### 12.3 Shard Local Admin

Shard 的 `127.0.0.1` 页面显示本机 agent ID、tracker 校准后的 UNIX time、当前
Project/shard/generation/status/owner lease、job 状态计数和 source load/checkpoint
recovery 错误。第一版唯一写操作是暂停/恢复本进程的新 claim；已有 attempt、heartbeat、
lease maintenance 和 checkpoint 不受影响。

Machine token、endpoint、pin 和 checkpoint 周期在启动配置中显式提供；Project/shard
生命周期只从 tracker admin 操作。本机页不能修改 owner、generation、checkpoint
pointer 或 SQLite job，也不实现进程控制、日志导出和立即 checkpoint 按钮。
...
2. Job Receiver 与第二阶段 merge/split 的真实规模、内存预算和默认 batch 参数；
3. 对浏览器无法访问 GitHub 授权页的用户，是增加独立的国内身份源，还是管理员一次性邀请码；第一版不代理 GitHub 登录页面、密码或 cookie；
4. 使用目标 tracker 机型、真实 JobSpec 和多台公网 shard 完成 100,000 completed jobs/s 验收时的 shard 数量、延迟与带宽门槛。
...
5. 实现 PostgreSQL 控制面、vnode session、assignment，以及 shard 的 attach/drain/pause/recover 最小状态机；
