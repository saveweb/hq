...
func (h *Handler) createProject(ctx *echo.Context) error {
	...
	dispatchQPS, workerClaimQPS, maxJobs, maxResets, recommendedLeaseSeconds, err := projectPolicyForm(ctx)
	if err != nil {
	...
	project := tracker.Project{
		...
		DispatchQPS: dispatchQPS, WorkerClaimQPS: workerClaimQPS, MaxJobsPerClaim: maxJobs, MaxResets: &maxResets,
...
func projectPolicyForm(ctx *echo.Context) (*float64, *float64, int, int, int64, error) {
	...
	if raw := strings.TrimSpace(ctx.FormValue("max_jobs_per_claim")); raw != "" {
		...
		if err != nil || maxJobs < 1 || maxJobs > 256 {
			return nil, nil, 0, 0, 0, fmt.Errorf("invalid max jobs per claim")
		}
	}
	maxResets := 3
	if raw := strings.TrimSpace(ctx.FormValue("max_resets")); raw != "" {
		...
		if err != nil || maxResets < 0 || maxResets > 1000 {
			return nil, nil, 0, 0, 0, fmt.Errorf("invalid max resets")
		}
	}
	recommendedLeaseSeconds := int64(300)
	if raw := strings.TrimSpace(ctx.FormValue("recommended_lease_seconds")); raw != "" {
		recommendedLeaseSeconds, err = strconv.ParseInt(raw, 10, 64)
		if err != nil || recommendedLeaseSeconds < 1 || recommendedLeaseSeconds > 3600 {
			return nil, nil, 0, 0, 0, fmt.Errorf("invalid recommended lease seconds")
		}
	}
	return dispatchQPS, workerClaimQPS, maxJobs, maxResets, recommendedLeaseSeconds, nil
}
