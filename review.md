...
- receiver、artifact/WARC、checkpoint 流量与 tracker/HQ 1 Gbps 的竞争。
...
### 2.4 Artifact/WARC 上传

Queue complete 只保存 `outcome_uri`，不允许把 WARC 或页面正文内嵌进 queue JSON。仍需单独确定：

- 普通 worker 如何在没有 R2 credential 的前提下上传 artifact；
- object identity 如何包含 project/shard/job/attempt，保证重试幂等；
- 失效 generation 已上传但未提交的对象如何 GC；
- artifact gateway 是否与 tracker 同机，以及它对 1 Gbps 带宽的影响。
...
8. 最后实现 R2 checkpoint/recovery、receiver、artifact gateway、archive、GC 和完整管理页。
